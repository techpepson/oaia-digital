...
const AuditCases = () => {
  ...
  const auditCases = [
    {
      ...
      description: 'Investigation of potential duplicate payments to ABC Construction Ltd',
    ...
    },
    {
      ...
      description: 'Review of contract amendments and cost variations exceeding approved limits',
      agency: 'Road Fund',
      priority: 'medium',
    ...
    },
    {
      ...
      description: 'Analysis of systematic budget overruns in infrastructure projects',
    ...
    },
    {
      ...
      description: 'Compliance audit of procurement processes and vendor selection',
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'on-hold': return <Pause className="h-4 w-4 text-gray-600" />;
  ...
  const filteredCases = auditCases.filter(auditCase => {
    const matchesSearch = auditCase.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         auditCase.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         auditCase.agency.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = statusFilter === 'all' || auditCase.status === statusFilter;
    const matchesPriority = priorityFilter === 'all' || auditCase.priority === priorityFilter;
    ...
    <div className="min-h-screen bg-oaia-light">
      ...
      <header className="bg-white border-b shadow-sm">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <span className="text-sm text-oaia-gray">Audit Cases Management</span>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-oaia-gray mt-1">Manage and track ongoing audit investigations</p>
        </div>
        ...
        <div className="space-y-6">
          {filteredCases.map((auditCase) => (
            <Card key={auditCase.id} className="hover:shadow-md transition-shadow">
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-4">
                    {getStatusIcon(auditCase.status)}
                    <div>
                      <CardTitle className="text-xl text-oaia-blue">{auditCase.id}</CardTitle>
                      <CardDescription className="text-lg font-medium text-gray-900 mt-1">
                        {auditCase.title}
                      </CardDescription>
                      <p className="text-sm text-oaia-gray mt-2">{auditCase.description}</p>
                    </div>
                  </div>
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center text-sm">
                      ...
                      <span className="ml-2 font-medium">{auditCase.dateOpened}</span>
                    </div>
                    <div className="flex items-center text-sm">
                      <Clock className="h-4 w-4 mr-2 text-oaia-gray" />
                      <span className="text-oaia-gray">Days Open:</span>
                      <span className="ml-2 font-medium">{auditCase.daysOpen} days</span>
                    </div>
                    <div className="flex items-center text-sm">
                      <FileSearch className="h-4 w-4 mr-2 text-oaia-gray" />
                      <span className="text-oaia-gray">Findings:</span>
                      <span className="ml-2 font-medium">{auditCase.findings} items</span>
                    </div>
                  </div>
        ...
        {filteredCases.length === 0 && (
          <Card className="text-center py-12">
            <CardContent>
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">No audit cases found</h3>
              <p className="text-oaia-gray">Try adjusting your search criteria or create a new audit case.</p>
            </CardContent>
          </Card>
        )}
      </div>
